...
└── backend/
    ...
    │   ├── types/         # definições de tipos globais do projeto
...
Os schemas usados pelo Swagger devem ser definidos diretamente nas rotas ou em arquivos específicos dentro de `src/routes/` ou `src/types/`, já que a pasta `src/docs/` não existe.

- Nunca definir schemas inline dentro do arquivo de rota — sempre importar de arquivos dedicados.
...
- Não pode haver duas colheitas registradas para a mesma data (implementado no `HarvestService`).
