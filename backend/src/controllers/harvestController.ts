...
export class HarvestController {
  async createHarvest(req: FastifyRequest, reply: FastifyReply) {
    try {
      const data = req.body as { plantationId: string; revenue: number; date: Date; userId?: string };
  ...
  async getHarvestsByPlantation(req: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const harvests = await harvestService.getHarvestsByPlantation(plantationId);
